...
class State:
    ...
    def getAllPossibleStates(self):
        ...
        if self.playerNo == 1:
            r, c = self.my_pos
            row = [np.max([r-self.max_step, 0]), np.min(r+self.max_step+1, self.chess_board.shape[0])]
            column = [np.max([c-self.max_step, 0]), np.min(c+self.max_step+1, self.chess_board.shape[0])]

            for i in np.arange(row[0], row[1]):
                for j in np.arange(column[0], column[1]):
